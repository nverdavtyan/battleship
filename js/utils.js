...
(function (global) {
    ...
    global.utils = {
        CELL_SIZE: 60, // retourne la position (démarre à 1) du noeud passé en paramètre dans son parent
        eq: function (node) {
        ...
        calculateGridMove: function (player) {
            let gridMove = 600;

            for (let i = 0; i < player.activeShip; i++) {
                let ship = player.fleet[i];
                gridMove = ship.dom.clientWidth > ship.dom.clientHeight ? gridMove + 60 : gridMove + 60 * ship.life;
            }
            return gridMove;
        },
    };
